...
const EMISOR = `${ORIGEN_IDP}/application/o/secretdrop`;
const CLIENT_ID = process.env.CLIENT_ID ?? "secretdrop-pruebas";
...
// Secretdrop no tiene un "quién soy": lo que hay es el listado de ficheros, que
// exige sesión. Sirve igual y además comprueba algo que la gente usa.
const puede = async () => (await fetch(`${BASE}/api/secrets`, { headers: { cookie: cabecera() } })).status;
