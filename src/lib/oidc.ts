...
export function oidcConfig(): OidcConfig | null {
  ...
  const issuer = validUrl(process.env.SECRETDROP_OIDC_ISSUER?.trim(), true);
  const internalOrigin = validUrl(
    process.env.SECRETDROP_OIDC_INTERNAL_BASE?.trim() || (issuer ? new URL(issuer).origin : undefined),
    false
  );
